        ...
        while(True):
            for i in range (a[1]):
                ...
                for j in range (a[2]):
                    ...
                    for k in range (a[3]):
                        del_middle(temp_middle[0,:,:,k],i,j,window_size)
            ...
            if criterion(ori_cost, infer, pix):
                flag_old_good = True
                if window_size == 0:
                    break
                elif flag_expand:
                    #print('git good                 ' + str(window_size))
                    break
                else:
                    window_size -= 1
                    #print('window decremented to '+ str(window_size))
            else:
                window_size += 1
                if flag_old_good:
                    infer = old_infer
                    break
                #print('window incremented to '+ str(window_size))
                flag_expand = True
            old_infer = infer.copy()
